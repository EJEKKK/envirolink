...
export default function VolunteerAttendanceDialog({
	...
	const handleOnAddScore = async (values: ScoreFormSchema) => {
		...
		// Retrieve a reference to the campaign manager's user document with a Firestore data converter
		const campaignManagerRef = doc(
			db,
			"users",
			campaign.managerUid,
		).withConverter(userConverter);

		const points = campaign.points || 0;
		...
		const campaignManagerData = (
			await getDoc(campaignManagerRef)
		).data() as User;
